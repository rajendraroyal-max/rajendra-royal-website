...
const G = {
  gold: "#b8963e", goldLight: "#d4aa56", goldBg: "rgba(184,150,62,.08)",
  goldBorder: "rgba(184,150,62,.26)", navy: "#0f1c2e", primary: "#0f1c2e",
  bg: "#f5f6f8", bgCard: "#ffffff", bgAlt: "#eceef2",
  text: "#0d1b2a", sub: "#3d4f63", muted: "#7a8fa8",
  border: "#dde3ec", borderGold: "rgba(184,150,62,.22)",
};
...
body{font-family:'DM Sans',sans-serif;background:${G.bg};color:${G.text};overflow-x:hidden;line-height:1.65;-webkit-font-smoothing:antialiased}
...
/* NAV */
nav{position:sticky;top:0;z-index:100;background:rgba(245,246,248,.97);backdrop-filter:blur(16px);border-bottom:1px solid ${G.border};height:66px;padding:0 5%;display:flex;align-items:center;justify-content:space-between;box-shadow:0 1px 12px rgba(0,0,0,.06)}
...
.sl{font-family:'DM Mono',monospace;font-size:.65rem;letter-spacing:.24em;text-transform:uppercase;color:${G.gold};margin-bottom:.8rem;display:flex;align-items:center;gap:.8rem}
.sl::after{content:'';display:block;height:1px;width:40px;background:${G.gold};opacity:.7}
.st{font-family:'Playfair Display',serif;font-size:clamp(1.8rem,3.5vw,2.8rem);font-weight:500;line-height:1.12;margin-bottom:1rem;color:${G.primary};letter-spacing:-.01em}
.st em{font-style:italic;color:${G.gold}}
.sd{font-size:.92rem;color:${G.sub};max-width:500px;line-height:1.8}

/* HERO */
.hero{background:linear-gradient(150deg,${G.primary} 0%,#0a1628 60%,#071020 100%);padding:80px 5% 72px;display:grid;grid-template-columns:1fr 360px;gap:4rem;align-items:center;position:relative;overflow:hidden}
